...
fn main() {
    ...
    println!(
        ...
        plain
            .get(prefix_len..(prefix_len + "foo@bar".len()))
            .unwrap(),
        plain
            .get((prefix_len + "foo@bar".len())..(prefix_len + "foo@bar".len() + middle_len))
            .unwrap(),
        plain
            .get((prefix_len + middle_len + "foo@bar".len())..)
            .unwrap()
    );
    ...
    let malicious_block_2 = encrypt(malicious_email, &key)
        .get(16..32)
        .unwrap()
        .to_owned();
    debug_assert_eq!(
        aes_ecb_decrypt(&malicious_block_2.to_vec(), &key),
        pad_pkcs7(b"admin".to_vec(), blocksize as u8)
    );
    ...
    let genuine_profile = encrypt(
        vec![0x41u8; blocksize - (prefix_len + middle_len) % blocksize],
        &key,
    );
    let mut evil_profile = genuine_profile
        .get(0..genuine_profile.len() - 16)
        .unwrap()
        .to_vec();
    ...
    assert!(
        decrypted_evil_profile.ends_with("role=admin"),
        "the profile says admin now"
    );
}
